...
export default function EntryLedgerPage(props: LedgerPageProps) {
			...
			<div className="flex flex-col h-full">
				<Form {...ledgerForm}>
					<form
						...
						onSubmit={(e) => {
							e.preventDefault()
							ledgerForm.handleSubmit((formData) => {
									...
									{ name: formData.name, currency: formData.currency },
									{
										onSuccess: (successData) => {
											...
											if (isUpdate) {
												...
												setCurPage(0)
												form.setValue("ledger", successData.data?.[0].id)
											}
										...
										<FormControl>
											{userQuery.isLoading || currenciesQuery.isLoading ? (
												...
												<ComboBox
													...
													onChange={(e) => {
														ledgerForm.setValue("currency", JSON.parse(e))
													}}
													values={
