...
function handleUpdate(
	...
	if (newEntryDate.getTime() > new Date(newEntries[0].date).getTime()) {
...
function groupData(data: Entry[]) {
	...
	for (let i = 1; i < data.length; i++) {
		...
		if (
			d.getMonth() === MONTHS.indexOf(lastGroup?.month) &&
			d.getFullYear() === lastGroup.year
		) {
			continue
		}
 ...
 */
function groupDataByMonth(data: Entry[]) {
...
function filterDataGroup(
	...
	while (l <= r) {
		...
		if (
			today.getMonth() === cur.getMonth() &&
			today.getFullYear() === cur.getFullYear()
		) {
