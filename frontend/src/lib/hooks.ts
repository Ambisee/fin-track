...
import { AuthError, PostgrestError, UserResponse } from "@supabase/supabase-js"
import {
...
function useEntryDataQuery() {
	...
	return useQuery({
		...
		queryFn: async () => {
			...
			if (!isNonNullable(settings)) {
				throw Error(QueryHelper.MESSAGE_NO_SETTINGS)
			}
			...
			const { data } = await sbBrowser
				...
				.eq("ledger", settings.current_ledger)
				.order("date")
				.order("category")
				.limit(100)
                .throwOnError()
